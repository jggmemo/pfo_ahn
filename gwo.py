...
#ModelEval_type = nb.deferred_type()
__all__ = ['GreyWolfOptimizer']

spec = [
    ('benchmark'),
    ('D', nb.int64),  # dimension of the problem Centers
    ('NP', nb.int64), # population size; number of search agents
    ('nFES', nb.int64), # number of function evaluations
    ('Lower', nb.float64[:]),  # lower bound
    ('Upper', nb.float64[:]),  # upper bound
    ('Fun'),
    ('Positions'),
    ('eval_flag', nb.bool_), # evaluations flag
    ('evaluations', nb.int64), # evaluations counter
    ('Alpha_pos', nb.float64[:]), # init of alpha
    ('Alpha_score', nb.float_),
    ('Alpha_ahn'),
    ('Beta_pos', nb.float64[:]),  # init of beta
    ('Beta_score', nb.float_),
    ('Delta_pos', nb.float64[:]), # init of delta
    ('Delta_score', nb.float_),
]
...
class GreyWolfOptimizer(object):
    ...
    **Reference paper:**
    ...
    def __init__(self, D, NP, nFES, benchmark):
        ...
        self.D = D  # dimension of the problem Centers
    ...
    def initialization(self):
        ...
        for i in range(self.NP):
            for j in range(self.D):
                self.Positions[i][j] = random.random() * (self.Upper[j] - self.Lower[j]) + self.Lower[j]
    ...
    def move(self):
        ...
        while self.eval_flag is not False:
            ...
            for i in range(self.NP):
                ...
                Fit, ahn = self.Fun(self.D, self.Positions[i]) #evaluacion
