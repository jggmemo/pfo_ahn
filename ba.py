...
class BatAlgorithm(object):
    ...
    **Reference paper:**
    ...
    def init_bat(self):
        ...
        for i in range(self.D):
            self.Lb[i] = self.Lower[i]
            self.Ub[i] = self.Upper[i]
        ...
        for i in range(self.NP):
            ...
            self.Fitness[i], self.ahn = self.Fun(self.D, self.Sol[i])
    ...
    def move_bat(self):
        ...
        while self.eval_flag is not False:
            for i in range(self.NP):
                ...
                Fnew, ahn_new = self.Fun(self.D, S[i])
                self.evaluations = self.evaluations + 1
                rnd = random.random()
